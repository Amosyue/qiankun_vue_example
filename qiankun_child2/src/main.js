...
function render(state,props = {}) {
  ...
  router = new VueRouter({
    base: window.__POWERED_BY_QIANKUN__ ? '/qiankun_child2' : '/',
    mode: 'hash',
    routes,
  });
...
export async function bootstrap(data) {
  console.log("***************进入child2项目bootstrap******************")
  console.log(data);
}
export async function mount(props) {
  Vue.use(props.data.components)
  // 挂载主项目传递过来的全局方法
  let $tools = {}
  Object.keys(props.data.libraryJs).forEach(i => {
    $tools[i] = props.data.libraryJs[i]
  });
  Vue.prototype.$tools = $tools
  console.log("***************进入child2项目mount******************")
  console.log(props);
